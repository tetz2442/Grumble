<?php
	...
	if(isset($_POST["name"]) && isset($_POST["email"]) && isset($_POST["message"]) && isset($_POST["type"]) && $_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if(strlen($name) < 50 || strlen($email) < 255 || strlen($message) < 500) {
			...
			$sql = "SELECT user_email FROM users_grumble WHERE access_lvl = 3";
			$result = mysql_query($sql, $conn) or die("Error: " . mysql_error());
			while($row = mysql_fetch_array($result)) {
				sendEmail($row["user_email"], "no-reply@grumbleonline.com", "admincontact");
			}
			echo 1;
		}
		else {
