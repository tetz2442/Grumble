<?php
session_start();
//check if user is logged in and time was passec
if(!isset($_SESSION["user_id"]) && isset($_POST["time"])) {
	...
	$offset = strip_tags($_POST['time']);
	//loop through each timezone
	foreach($zonelist as $key => $value) {
		$timezone = new DateTimeZone($key);
		$offsets   = $timezone->getOffset(new DateTime) / 60 / 60;
		//check if offset matches, if it does, that is the timezone they are in
        if($offsets == $offset) {
        	$_SESSION['time'] = $key;
        	break;
        }
